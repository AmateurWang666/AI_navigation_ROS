...
一直跑下去。这条要求正好排除了 rclpy 自带的信号处理——它会在节点的清理代码
有机会发布任何东西之前，就先把 context 拆掉。
"""
...
def install_shutdown_signals():
    ...
    会被置为 SIG_IGN；此时 rclpy 的处理器又已被禁用，进程就会完全忽略 Ctrl-C。
    SIGTERM 同样要接：launch 在节点迟迟不退出时会升级到 SIGTERM，而 Python
    默认把它当作静默 kill，一点刹车的机会都没有。
    """
